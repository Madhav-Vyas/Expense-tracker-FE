 ...
 * Expansive, highly visual financial command center with 3 tiers:
...
const AnalyticsDashboard = ({ selectedMonth, selectedYear }) => {
  ...
  if (isLoading) {
    return (
      <div className="w-full space-y-6 animate-pulse">
        <div className="h-14 bg-slate-900/60 border border-slate-800/80 rounded-2xl"></div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="h-44 bg-slate-900/50 rounded-2xl border border-slate-800/80"></div>
          <div className="h-44 bg-slate-900/50 rounded-2xl border border-slate-800/80"></div>
          <div className="h-44 bg-slate-900/50 rounded-2xl border border-slate-800/80"></div>
        </div>
        <div className="h-80 bg-slate-900/40 rounded-3xl border border-slate-800/80"></div>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="w-full text-center py-16 bg-rose-950/20 border border-rose-500/20 rounded-3xl p-8 backdrop-blur-xl">
        <IconExclamationCircle className="mx-auto text-rose-400 mb-3" size={42} />
        <h3 className="text-xl font-bold text-white">Analytics Engine Offline</h3>
  ...
  return (
    <div className="w-full space-y-7">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/40 border border-slate-800/80 p-2.5 rounded-2xl backdrop-blur-xl">
        <div className="flex items-center gap-1.5 flex-wrap">
          {[
            { id: "all", label: "Executive Overview", icon: IconLayersLinked },
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 cursor-pointer ${
                  isActive
                    ? "bg-gradient-to-r from-violet-600/90 to-cyan-600/90 text-white shadow-lg shadow-violet-600/20 border border-violet-400/30"
      ...
      {(activeView === "all" || activeView === "categories") && (
        <div className="relative overflow-hidden bg-slate-900/50 border border-slate-800/80 rounded-3xl p-7 shadow-2xl backdrop-blur-xl">
          <div className="absolute top-0 right-0 w-96 h-96 bg-violet-600/10 blur-[100px] pointer-events-none -z-10 rounded-full" />

          {/* Section Header */}
          <div className="flex flex-col md:flex-row md:items-center justify-between pb-5 mb-6 border-b border-slate-800/60 gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                ...
                <span className="text-lg font-black text-rose-400">{formatUSD(totalExpense)}</span>
              </div>
            </div>
          </div>
                ...
                {categoriesBreakdown.map((cat, idx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`${segmentColor} hover:brightness-125 transition-all duration-300 relative group cursor-pointer`}
                    />
                  );
                })}
              </div>
          ...
          {categoriesBreakdown.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-16 h-16 rounded-full bg-slate-950/80 border border-slate-800 flex items-center justify-center text-3xl mb-3">
                📊
              </div>
              <p className="text-sm font-bold text-slate-300">No Expenses Recorded for {currentMonthLabel}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {categoriesBreakdown.map((cat, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-4 rounded-2xl bg-slate-950/50 border border-slate-800/60 hover:border-slate-700 hover:scale-[1.01] transition-all duration-300 group shadow-lg"
                  >
      ...
      {(activeView === "all" || activeView === "budget") && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-7">
          ...
          <div className="lg:col-span-7 relative overflow-hidden bg-slate-900/50 border border-slate-800/80 rounded-3xl p-7 shadow-2xl backdrop-blur-xl flex flex-col justify-between">
            ...
            <div>
              <div className="flex items-center justify-between pb-5 mb-6 border-b border-slate-800/60">
                ...
                <span className="text-[10px] px-3 py-1 bg-cyan-500/10 text-cyan-400 font-bold border border-cyan-500/20 rounded-lg">
                  Standard Rule
                </span>
              </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/60 space-y-2.5">
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-slate-200">Needs (Bills, Health, Domestic Staff)</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-extrabold text-white">{formatUSD(budgetNeeds.amount)}</span>
                      <span className="text-slate-400">({budgetNeeds.percentage}% / <b className="text-cyan-400">50% Target</b>)</span>
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/60 space-y-2.5">
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-slate-200">Wants (Food, Entertainment, Shopping)</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-extrabold text-white">{formatUSD(budgetWants.amount)}</span>
                      <span className="text-slate-400">({budgetWants.percentage}% / <b className="text-rose-400">30% Target</b>)</span>
                    </div>
                  </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/60 space-y-2.5">
                  <div className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-slate-200">Investments (SIPs, Gold, Savings)</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-extrabold text-white">{formatUSD(budgetInvestments.amount)}</span>
                      <span className="text-slate-400">({budgetInvestments.percentage}% / <b className="text-violet-400">20% Target</b>)</span>
                    </div>
                  </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-400">
              ...
              <span className={`font-black text-sm ${netSavings >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="lg:col-span-5 relative overflow-hidden bg-slate-900/50 border border-slate-800/80 rounded-3xl p-7 shadow-2xl backdrop-blur-xl flex flex-col justify-between">
            <div className="absolute top-0 right-0 w-80 h-80 bg-amber-500/10 blur-[90px] pointer-events-none -z-10 rounded-full" />

            <div>
              <div className="flex items-center justify-between pb-5 mb-5 border-b border-slate-800/60">
                <div className="flex items-center gap-3">
                  <div className="p-2.5 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20 text-lg">
                    🇮🇳
                  </div>
                  <div>
                    <h3 className="text-xl font-extrabold text-white tracking-tight">
                ...
                <span className="text-[10px] px-2.5 py-1 bg-amber-500/10 text-amber-400 font-bold border border-amber-500/20 rounded-lg">
                  Lifestyle Drain
                </span>
              </div>

              {/* Aggregate Total Banner */}
              <div className="p-4 rounded-2xl bg-slate-950/70 border border-slate-800/70 text-center mb-5">
                ...
                <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-slate-800/50 text-left">
                  <div className="flex items-center gap-2">
                    <span className="p-1.5 rounded-lg bg-amber-500/10 text-amber-400">
                      <IconCoffee size={14} />
                    </span>
                    <div>
                      <span className="text-[9px] text-slate-400 font-bold uppercase block">Quick Delivery</span>
                      <span className="text-xs font-black text-white">{formatINR(lifestyle?.convenienceTax?.quickCommerce || 0)}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <span className="text-[9px] text-slate-400 font-bold uppercase block">Cabs & Rides</span>
                      <span className="text-xs font-black text-white">{formatINR(lifestyle?.convenienceTax?.rideHailing || 0)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs font-bold text-slate-300 mb-2">
                  ...
                  <span className="text-amber-400">{convenienceTaxList.length} Found</span>
                </div>

                {convenienceTaxList.length === 0 ? (
                  <div className="text-center py-8 bg-slate-950/40 rounded-2xl border border-slate-850">
                    <p className="text-xs font-bold text-slate-400">Zero Convenience Spends</p>
                    <p className="text-[10px] text-slate-500 mt-0.5">No delivery apps or cab charges logged this month.</p>
                  </div>
                ) : (
                  <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1 scrollbar-thin">
                    {convenienceTaxList.map((tx) => {
                      ...
                      return (
                        <div
                          key={tx._id}
                          className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950/60 border border-slate-850 hover:border-slate-800 transition-colors"
                        >
                          <div className="flex items-center gap-2.5">
                            <span className={`p-1.5 rounded-lg ${isFood ? 'bg-amber-500/10 text-amber-400' : 'bg-blue-500/10 text-blue-400'}`}>
                              {isFood ? <IconCoffee size={14} /> : <IconCar size={14} />}
                            </span>
                            <div className="text-left">
                              <p className="text-xs font-bold text-slate-200 capitalize truncate max-w-[170px]">
                                {tx.description}
                              </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-7 items-start">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-5 rounded-2xl bg-slate-950/60 border border-slate-800/70 text-center relative overflow-hidden">
                  <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider block">
                  ...
                  <div className="flex items-baseline justify-center gap-1.5 mt-2">
                    ...
                    <span className="text-sm font-bold text-slate-400">Months</span>
                  </div>
              ...
              <div className="p-5 rounded-2xl bg-slate-950/60 border border-slate-800/70 space-y-3.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <IconAdjustmentsHorizontal size={16} className="text-violet-400" />
                    <span className="text-xs font-bold text-slate-200">Interactive Burn Rate Simulator</span>
                  </div>
                ...
                <div className="pt-2">
                  <input
                    ...
                    className="w-full h-2 bg-slate-900 rounded-lg appearance-none cursor-pointer accent-violet-500 focus:outline-none"
                  />
            ...
            <div className="lg:col-span-5 p-5 rounded-2xl bg-slate-950/60 border border-slate-800/70 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800/50 pb-3">
                <div className="flex items-center gap-2">
                  <IconReceipt size={16} className="text-emerald-400" />
              ...
              {(!forecast?.projectedBills || forecast.projectedBills.length === 0) ? (
                <div className="text-center py-10">
                  <IconShieldCheck className="mx-auto text-slate-600 mb-2" size={28} />
                  <p className="text-xs font-bold text-slate-400">No Recurring Bills Detected</p>
                ...
                <div className="space-y-2.5 max-h-[260px] overflow-y-auto pr-1 scrollbar-thin">
                  {forecast.projectedBills.map((bill, idx) => (
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 rounded-xl bg-slate-900/40 border border-slate-850 hover:border-slate-800 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left space-y-0.5">
                          <p className="text-xs font-bold text-slate-200 capitalize truncate max-w-[150px]">
                            {bill.name}
                          </p>
